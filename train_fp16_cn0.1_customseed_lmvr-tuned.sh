...
train() {
    ...
    echo "================ FLORES BASELINE WITH CLIP_NORM=0.1, BPE=$BPE_SIZE AND SEED=$RAND_SEED USING TUNED LMVR ================" >>$LOG_OUTPUT_PATH
    ...
    if [ "$SRC_LANG" = "si" ] || [ "$TGT_LANG" = "si" ]; then
        DATA_DIR="data-bin/wiki_si_en_lmvr-tuned/"
    else
        DATA_DIR="data-bin/wiki_ne_en_lmvr-tuned/"
    fi
